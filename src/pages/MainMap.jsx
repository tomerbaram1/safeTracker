...
export default function MainMap() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((data) => {
        setKidsLocations(data.data);
        setPin({
          latitude:
            data.data[0].location[data.data[0].location.length - 1].latitude,
          longitude:
            data.data[0].location[data.data[0].location.length - 1].longitude,
        });
      });
  }, []);
  ...
  function changeRegion(child) {
    const latitude = parseFloat(
      child.location[child.location.length - 1].latitude
    );
    const longitude = parseFloat(
      child.location[child.location.length - 1].longitude
    );
